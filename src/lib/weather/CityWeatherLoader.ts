...
import { CityCodes as cityCodes } from './CityCodes.ts';
...
export class CityWeatherLoader {
  ...
  #getSummary(body: string): { img: string; weather: string } {
    ...
    const $target = $('div.item-wrap ul.item').filter((_, element) => {
      const $e = $(element);
      const parsedTime = $e.data('time') as string;
      const parsedHour = parseInt(parsedTime.replace(/(\d{2}):\d+/, '$1'));
      const dateStr = $e.data('date') as string;
